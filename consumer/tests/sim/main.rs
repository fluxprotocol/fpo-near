...
fn simulate_get_price() {
    ...
    call!(
        provider1,
        fpo.create_pair("ETH/USD", 8, U128(2000))
    )
    .assert_success();
    call!(
        provider1,
        fpo.pair_exists("ETH/USD", provider1.account_id())
    )
    .assert_success();
    let price_entry = call!(
        provider1,
        fpo.get_entry("ETH/USD", provider1.account_id())
    );
    ...
    let outcome = call!(
        provider2,
        consumer.get_price("ETH/USD", provider1.account_id())
    );
...
fn simulate_get_prices() {
    ...
    call!(
        provider2,
        fpo.create_pair("ETH/USD", 8, U128(4000))
    )
    .assert_success();
    call!(
        provider2,
...
fn simulate_get_price_call() {
    ...
    let fetched_entry = call!(
        user,
        consumer.get_pair(provider1.account_id(), "ETH/USD")
    );
...
fn simulate_get_prices_call() {
    ...
    let price_entry = call!(
        provider2,
        fpo.get_entry("BTC/USD", provider2.account_id())
    );
    ...
    let fetched_entry = call!(
        user,
        consumer.get_pair(provider2.account_id(), "BTC/USD")
    );
...
fn simulate_get_prices_call2() {
    ...
    let fetched_entry = call!(
        user,
        consumer.get_pair(provider2.account_id(), "ETH/USD")
    );
